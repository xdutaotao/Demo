...
static int i =0;
		...
		myThread = new Thread( new Runnable()
			{
				public void run()
				{
					...
					{
						myOut.println( "jack"+ "accet is error" );
		...
		**/
			...
			throws InterruptedException
		{
			try {
				...
				String path = AudioActivity_Player.info.getSDsrc();
				uri = path.substring(path.lastIndexOf("/") + 1);
							   ...
							   boolean allowDirectoryListing )
	{
		...
		String path = AudioActivity_Player.musicList.get(AudioActivity_Player.Selected).getSDsrc();
		homeDir = new File(path.substring(0, path.lastIndexOf("/") + 1));
